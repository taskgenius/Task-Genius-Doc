...
Enable/disable progress bars on regular list items (`addProgressBarToNonTaskBullet`), headings (`addTaskProgressBarToHeading`), or only task items.

### Conditional hiding

Hide progress bars based on specific tags (`hideProgressBarTags`), parent tags, file paths (`hideProgressBarFolders`), or frontmatter metadata (`hideProgressBarMetadata`).
